...
export default function Navbar({ email }: { email: string | null }) {
    ...
    const UserImage = <Image src="/user.svg" height={25} width={25} alt="Profile" className="cursor-pointer mr-5 hover:scale-110 transtion-all duration-300" />
    return (
        <AppBar position="sticky" className="font-bold bg-gradient-to-r from-blue-500 to-blue-300 shadow-lg">
            <Toolbar>
                <div className="flex flex-row items-center justify-evenly w-full h-full">
                    <Link className="hover:scale-110 transition-all duration-300" href="/"><InsertChartIcon className="mr-2 fill-slate-800" />Market</Link>
                    <Link className="hover:scale-110 transition-all duration-300" href="/explorer"><ArticleIcon className="mr-2 fill-slate-800" />Explorer</Link>
                    {email ? <Link className="hover:scale-110 transition-all duration-300" href="/portfolio"><WorkIcon className="mr-2 fill-gray-600" />Portfolio</Link> : null}
                </div>
